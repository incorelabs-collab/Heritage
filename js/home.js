var pageHome = {
    ...
    tokenHandlerAPN: function(token) {
        ...
        if(pushToken == null || pushToken != token) {
            $.ajax({
                url: 'http://heritage.incorelabs.com/notification/register.php',
                type: 'POST',
                dataType: 'json',
    ...
    launchAlbumsPage: function() {
        if(app.isConnectionAvailable()) {
            if (device.platform == 'android' || device.platform == 'Android') {
                pageHome.gallery_ref = window.open("http://heritage.incorelabs.com/gallery/index.php", "_blank","location=no,hidden=yes,zoom=no");
            } else {
                pageHome.gallery_ref = window.open("http://heritage.incorelabs.com/gallery/index.php", "_blank","location=no,closebuttoncaption=Close,hidden=yes");
            }
